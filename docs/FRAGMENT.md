...
| Layer | Fragment |
|---|---|
| Terms | logical variables, literals — including an integer literal at the `int` code — a module global read at a memory (`g{&m}`), and the result of the enclosing judgement (`res{1}`, `res{2}`, `res`). The boolean operators, equality, pairs and projections, `if` and `let` are terms, as are the integer operations of the expression layer, so `res + 1 = 1` is a term. A term at a function code is a lambda or an operator read, and an applied one is an application: this is what carries the predicate of a list combinator or of `choiceb`. Beyond those: an operator the ingestion's tables commit to a meaning for, applied to a term; the definite description `choiceb` at a predicate and a default; the iterators `iter` and `iterop`; the option former and its elimination; and `map`, `filter`, `foldr`, `all`, `has`, `count` and `uniq` on lists |
...
| A real bound outside a rational numeral, such as `q / 2^n` | reals appear only as probabilities, and a probability constant is a numeral or the reciprocal of one. An exponent that is a program or logical variable needs a probability former carrying a term, which the layer does not have |
