...
| Layer | Fragment |
|---|---|
| Types | `unit`, `bool`, a finite scalar type (`fin n`, interpreted as `Fin (n+1)`), products, `int`, and finite maps (`map a b`, an association list). `EcTy.isFin` marks the finite subset — `unit`, `bool`, `fin n`, and a product of two finite codes — which is where uniform sampling and a `Location` live |
